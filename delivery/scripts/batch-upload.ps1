...
$videos = @(
    @{slug="kayla-jason"; id="1097652265"},
...
foreach ($v in $videos) {
    ...
    Write-Host "[3/5] Transcoding 4K..."
    & ffmpeg -i "$dlDir\$slug.mp4" -c:v libx264 -preset slow -crf 18 -s 3840x2160 -c:a aac -b:a 192k -hls_time 6 -hls_playlist_type vod -hls_segment_filename "$outDir\hls\teaser\4k\segment_%03d.ts" "$outDir\hls\teaser\4k\playlist.m3u8" -y 2>&1 | Out-Null
    Write-Host "  4K done"
    
    Write-Host "  Transcoding 1080p..."
    & ffmpeg -i "$dlDir\$slug.mp4" -c:v libx264 -preset slow -crf 20 -s 1920x1080 -c:a aac -b:a 128k -hls_time 6 -hls_playlist_type vod -hls_segment_filename "$outDir\hls\teaser\1080p\segment_%03d.ts" "$outDir\hls\teaser\1080p\playlist.m3u8" -y 2>&1 | Out-Null
...
"@ | Set-Content "$outDir\hls\teaser\master.m3u8" -Encoding UTF8
    ...
    & ffmpeg -i "$dlDir\$slug.mp4" -ss 00:00:15 -frames:v 1 -q:v 2 "$outDir\thumb.jpg" -y 2>&1 | Out-Null
...
# Notify Iris
& openclaw system event --text "Done: All 12 remaining wedding teasers downloaded, transcoded to HLS (4K+1080p), thumbnails extracted, and uploaded to R2. All 13 total couples now on R2." --mode now

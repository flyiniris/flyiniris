...
foreach ($v in $videos) {
    ...
    if (-not $dl) { $dl = $video.download | Sort-Object width -Descending | Select-Object -First 1 }
...
"@ | Set-Content "$outDir\hls\teaser\master.m3u8" -Encoding UTF8
    ...
    if ($verify -match "master.m3u8") { Write-Host "  VERIFIED on R2" } else { Write-Host "  WARNING: Verification failed!" }
...
Write-Host "`n==============================="
Write-Host "ALL $total REMAINING VIDEOS COMPLETE"
Write-Host "==============================="

& openclaw system event --text "Done: All 13 wedding films are now on R2 with 4K+1080p HLS and thumbnails. Ready for review." --mode now

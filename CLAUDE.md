...
State as of 2026-09-26: the inquiry-first funnel (below) is live; `start.html` serves /start, the guided inquiry flow and ad destination (live 2026-07-12); `films.html` is the public portfolio page; couple delivery pages are deployed for amanda-boris, rachel-michael-street and matt-haley (matt-haley also loads its video list from prep.flyiniris.com/api/delivery/:slug/videos); `venues/` pages exist; on-site film thumbnails link to `/yours?film={slug}`, and `/watch` is a 200 rewrite to the same film player (`yours.html`, renamed 2026-09-15; `/yours` keeps working). The film player strips personal query params (fn, vn, wd, fc, fv) before GA4 and the pixel load (2026-09-26). The old video-match worker is archived at `delivery/archive/video-match/` (shadowed from serving); live matching runs in the iris-automation Worker.
...
The site is an inquiry-first funnel. Exact pricing is NEVER shown to anonymous visitors (copy, schema, and page-source JS all stay price-free). TWO standing exceptions: /start's Team-available screen shows "starting at $2,800" (Sean-approved 2026-07-10), and the homepage inquiry form shows "Collections start at $2,800." under its subtext (since 6495dd3, 2026-08-24). Both are static copy mirroring TEAM_BASE in pricing.ts; when pricing.ts changes, both follow.
- start.html is /start, the guided inquiry flow (live 2026-07-12, submit-at-vibe rework 2026-07-13): one question per screen, live availability routing via /api/availability (open / Team pitch / both-booked waitlist with submissionType 'waitlist'), film lightbox on the teaser HLS masters. The real /webhook/inquiry POST fires at vibe completion, phoneless (phone '' and smsNonMarketing 'No'); the phone screen is a post-submit OPTIONAL step with no step number, no back button, and a skip ghost, and it calls POST /api/inquiry/enrich-phone fire-and-forget (all response cases advance to confirm; the receipt line mentions a text only when the response said smsSent true). A no-date ghost on the date screen skips the availability check and submits weddingDate blank (Worker dateless handling); that path gets its own confirm H1 and "Details in" strip step. Progress is n/3 with vibe the final step. The welcome screen plays hero-9x16-v3.mp4 as a poster-first strip (WELCOME_VIDEO in page source is the one-line swap point for a future ad master). Tracking mirrors index (pixel + CAPI dedup event_id, GA4, Clarity); the pre-push tag manifest covers start.html and 404.html. ?offer=doc gates the welcome documentary-offer chip and a quiet echo line on the capture screen (session-only read, never persisted); the confirm screen's offer closer renders only while /api/pricing/constants (fetched post-submit) reports fomoFreeSpot true, with the live spot count (FOMO scarcity copy is deliberate marketing: never change it, Sean 2026-09-25). No pre-call copy may promise a date hold (booking a call holds nothing; only a post-call S&S soft hold exists).
- index.html renders the short inquiry form at #quiz (#inquiry alias): first names, email, phone, wedding date (inline availability check), venue optional, a REQUIRED "How did you find us?" select (#qf-found; the four referral answers reveal an optional Referral Name), one non-marketing SMS consent checkbox, and a Turnstile widget (submits go through the Turnstile post helper).
...
- thanks.html is the router: availability payoff + film match, PRIMARY embedded booking widget (book.flyiniris.com/widget/booking/Kd7zWqsXzAswGHR1HuDR), SECONDARY the couple's /yours page when the inquiry response carried yours_url (localStorage fi_yours_url, pinned to the token by fi_yours_url_for), else /calculator/?session=TOKEN&from=thanks (/start's confirm uses from=start_confirm; the calculator forwards from to its /api/session lookup). Homepage submits of a both-booked date go as submissionType 'waitlist' like /start, and /thanks swaps its copy via fi_waitlist_for.

...
- start.html is /start, the guided inquiry flow (live 2026-07-12): one question per screen, live availability routing via /api/availability (open / Team pitch / both-booked waitlist with submissionType 'waitlist'), film lightbox on the teaser HLS masters, then the SAME /webhook/inquiry submit and /thanks?session=TOKEN handoff as index. Tracking mirrors index (pixel + CAPI dedup event_id, GA4, Clarity); the pre-push tag manifest covers start.html and 404.html. ?offer=doc gates the welcome documentary-offer chip (session-only read, never persisted); the confirm screen's offer closer renders only while /api/pricing/constants (fetched post-submit) reports fomoFreeSpot true, with the live spot count.
